<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="bdg:SourceRoot[@xsi:type != 'listComplexElement']">
    ...
    <xsl:choose>
      <!-- #A&P 775175 mappView | Regression Runtime bindings are not created for complex bindings with source and target type = "breaseComplex" -->
      <xsl:when test="@xsi:type = 'opcUaComplex'">
      ...
      <xsl:otherwise>
        <xsl:attribute name="xsi:type">brease</xsl:attribute>
        <xsl:attribute name="refId">
    ...
    <!--samplingRate, serverAlias, namespaceAlias are currently not supported by schema-->
  </xsl:template>
